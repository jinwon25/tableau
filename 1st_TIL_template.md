...
![image](https://github.com/user-attachments/assets/bf1391a3-46ac-42a6-9c23-269e1e007df0)
...
```
라이브는 Tableau가 데이터 원본에 실시간으로 직접 연결하여 데이터를 가져오는 방식으로, 원본 데이터가 변경되면 시트에 즉시 반영됩니다. 그러나 데이터 양이 많아질수록 처리 속도와 성능이 저하될 수 있어, 온라인 환경에서 원본 데이터가 가볍고 자주 변동되는 상황에 유용합니다. 반면, 추출은 데이터 원본을 hyper 파일로 저장해 사용하며, 원본 데이터가 변경되면 새로고침을 통해 추출 데이터를 최신 상태로 유지합니다. 이로 인해 오프라인 작업이 가능하고, 라이브보다 처리 속도가 빠르기 때문에 대량의 데이터를 다루기에 적합합니다.
```
